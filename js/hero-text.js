...
function drawText() {
  ...
  ctx.font = `600 ${fontSize * dpr}px system-ui`;
...
const material = new THREE.ShaderMaterial({
  ...
  fragmentShader:`
    ...
    void main(){
      ...
      float n = noise(vUv * 900.0);
      float local = clamp((u_progress - vUv.x) / 0.22, 0.0, 1.0);
      if (n > local) discard;
...
function animate(ts){
  ...
  material.uniforms.u_progress.value =
    easeOut(Math.min(1,(ts-start)/1000/DURATION));
